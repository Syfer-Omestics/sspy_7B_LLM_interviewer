...
import robotAvatar from '@/assets/aichat_logo.svg'
interface Props {
  image?: boolean
}
defineProps<Props>()
...
<template>
  <template v-if="image">
    <NAvatar v-if="isString(avatar) && avatar.length > 0" :src="avatar" :fallback-src="defaultAvatar" color="white"/>
    <NAvatar v-else round :src="defaultAvatar" color="white" />
  </template>
  <NAvatar v-else round :src="robotAvatar" color="white" />
